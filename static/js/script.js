...
async function sendMessage() {
    ...
    .then(data => {
        ...
        // Handle base64 image response from GPT Image Generation
        if (data.image_url) {
...
selectBtn.onclick = function() {
    ...
    fetch(data.image_url)
        ...
        .then(blob => {
            ...
            // Add the new file to our global selectedFiles array
            selectedFiles.push(file);
            
            // Update the input files to reflect all selected files
            updateInputFiles();
            
            // Rebuild the preview to show all images
            previewImage();

            document.getElementById("user-input").value = currentInput;
